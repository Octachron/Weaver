...
let scale_geometry ~ctx geometry =
  ctx##(scale geometry.w geometry.h)

let draw_image ~ctx img (w,h) (x,y) =
  ...
  let gi = inv g in
  let p = gi %% p  in
  scale_geometry ~ctx g;
  ctx##drawImage img p.w p.h;
  scale_geometry ~ctx gi
...
let with_style ~ctx ~f style =
  ctx##.fillStyle := style; f ctx; ctx##.fillStyle := black

let draw_circle ~ctx ~radius ~pos:(x,y,z) =
...
let draw_quad ~ctx ~close (x0,y0) (tx1,ty1) (tx2,ty2) =
  ctx##beginPath;
  ctx##moveTo x0 y0;
...
let draw_para ~ctx ~close start v1 v2 v3 =
    let quad = draw_quad ~ctx ~close start in
...
let draw_cube ~ctx =
  ...
  draw_para ~ctx ~close start l r d
 ...
 let grab_ctx name ctx_r geometry_r =
  ...
  let mctx = Js.Opt.map canvas (fun canvas ->
    (canvas##getContext Dom_html._2d_, canvas##.width, canvas##.height)
  ) in
  Js.Opt.iter mctx (fun (ctx_v,w,h) ->
    let w, h = float_of_int w, float_of_int h in
    let g = {w;h} in
    Utils.may (fun ctx -> ctx##.fillStyle := Js.string "black") !ctx_r;
    ctx_r := Some ctx_v;
    geometry_r := g ;
    scale_geometry ~ctx:ctx_v (min_g g);
