...
let paint_half ~ctx = let open Plot in
  ...
  draw_para ~ctx ~close (0.5,0.5)  l r d;
  ctx##.fillStyle := blueish;
  draw_para ~ctx ~close (0.5+.xd,0.5+.yd) l r d;
  ctx##.fillStyle := black ;
  draw_quad ~ctx ~close:(fun ctx -> ctx##stroke) (0.5+.xd,0.5+.yd) l d
...
let draw ~ctx {n;config} =
  iter_n n (draw_p ~ctx config)

let draw_counter ~ctx env =
  let open Plot in
...
let draw_full ~ctx env =
  let open Plot in
  clear ~ctx;
  draw_cube ~ctx;
  draw ~ctx env;
  paint_half ~ctx;
  draw_counter ~ctx env

let rec update_0 ()=
  update_g env.config !dt;
  Utils.may_ctx (draw_full env) !ctx;
...
let do_step () =
  ...
  let () =
    ...
    Utils.may_ctx (draw_full env) !ctx in
  timer:= steps.(!step_r) ()
